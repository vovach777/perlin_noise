...
namespace perlin {
   inline auto permutation = std::vector<int>{ 151,160,137,91,90,15,
...
int main() {
    ...
    for (int h = 0; h < 25; ++h) 
    {
        for (int w = 0; w < 80; ++w) 
        {
            //std::cout << std::setw(10) << perlin::noise( w / 256.0, h / 256.0,  (h*w) / (16.0*16.0)) ;
            //std::cout << std::setw(5) << perlin::noise( w / 256.0, h / 256.0,  (h*w) / (16.0*16.0)) + 1;
            std::cout << std::setw(3) << int( perlin::noise( w / 80.0, h / 80.0, rand() % 256 ) * 9.0) + 8;

            //std::cout << std::setw(1) << pixels[ int(perlin::noise( sin(w*h / (80.0*25.0) * 2.0 * M_PI), 0,0 ) * 5.0) % 4  ] ;
        }
        std::cout << std::endl;
    }
}
